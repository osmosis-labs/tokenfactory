...
@test "tokenfactory successfully registered" {
  run echo $SIMD_MAIN_CMD
  run $SIMD_MAIN_CMD query staking params
  refute_line "denom: stake"
  refute_line 'amount: "1000000"'
  refute_line 'denom: stake'
}
...
#   sval_strd_balance_start=$($SIMD_MAIN_CMD  q bank balances $(STRIDE_ADDRESS) --denom $STRIDE_DENOM   | GETBAL)
#   hval_strd_balance_start=$($HOST_MAIN_CMD    q bank balances $HOST_VAL_ADDRESS --denom $IBC_STRD_DENOM | GETBAL)
#   sval_token_balance_start=$($SIMD_MAIN_CMD q bank balances $(STRIDE_ADDRESS) --denom $HOST_IBC_DENOM | GETBAL)
#   hval_token_balance_start=$($HOST_MAIN_CMD   q bank balances $HOST_VAL_ADDRESS --denom $HOST_DENOM     | GETBAL)

#   # do IBC transfer
#   $SIMD_MAIN_CMD tx ibc-transfer transfer transfer $STRIDE_TRANFER_CHANNEL $HOST_VAL_ADDRESS ${TRANSFER_AMOUNT}${STRIDE_DENOM} --from $STRIDE_VAL -y 
...
#   # get new balances
#   sval_strd_balance_end=$($SIMD_MAIN_CMD  q bank balances $(STRIDE_ADDRESS) --denom $STRIDE_DENOM   | GETBAL)
#   hval_strd_balance_end=$($HOST_MAIN_CMD    q bank balances $HOST_VAL_ADDRESS --denom $IBC_STRD_DENOM | GETBAL)
#   sval_token_balance_end=$($SIMD_MAIN_CMD q bank balances $(STRIDE_ADDRESS) --denom $HOST_IBC_DENOM | GETBAL)
...
#   token_balance_start=$($SIMD_MAIN_CMD   q bank balances $(STRIDE_ADDRESS) --denom $HOST_IBC_DENOM | GETBAL)
#   sttoken_balance_start=$($SIMD_MAIN_CMD q bank balances $(STRIDE_ADDRESS) --denom st$HOST_DENOM   | GETBAL)
...
#   # liquid stake
#   $SIMD_MAIN_CMD tx stakeibc liquid-stake $STAKE_AMOUNT $HOST_DENOM --from $STRIDE_VAL -y 
...
#   token_balance_end=$($SIMD_MAIN_CMD q bank balances $(STRIDE_ADDRESS) --denom $HOST_IBC_DENOM | GETBAL)
...
#   sttoken_balance_end=$($SIMD_MAIN_CMD q bank balances $(STRIDE_ADDRESS) --denom st$HOST_DENOM | GETBAL)
...
#   # call redeem-stake
#   $SIMD_MAIN_CMD tx stakeibc redeem-stake $REDEEM_AMOUNT $HOST_CHAIN_ID $HOST_RECEIVER_ADDRESS \
...
#   EPOCH=$($SIMD_MAIN_CMD q records list-user-redemption-record  | grep -Fiw 'epoch_number' | head -n 1 | grep -o -E '[0-9]+')

#   # claim the record (send to stride address)
#   $SIMD_MAIN_CMD tx stakeibc claim-undelegated-tokens $HOST_CHAIN_ID $EPOCH $(STRIDE_ADDRESS) \
#     --from $STRIDE_VAL --keyring-backend test --chain-id $STRIDE_CHAIN_ID -y
...
#   MULT=1000000
#   redemption_rate=$($SIMD_MAIN_CMD q stakeibc show-host-zone $HOST_CHAIN_ID | grep -Fiw 'redemption_rate' | grep -Eo '[+-]?[0-9]+([.][0-9]+)?')
...
#   val_address=$($SIMD_MAIN_CMD keys show ${STRIDE_VAL_PREFIX}2 --keyring-backend test -a)
#   $SIMD_MAIN_CMD tx distribution withdraw-all-rewards --from ${STRIDE_VAL_PREFIX}2 -y 
#   WAIT_FOR_BLOCK $STRIDE_LOGS 2

#   # confirm they've recieved stTokens
#   sttoken_balance=$($SIMD_MAIN_CMD q bank balances $val_address --denom st$HOST_DENOM | GETBAL)
